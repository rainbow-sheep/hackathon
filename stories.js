...
async function main() {
    const uids = (await axios.get(API_URL + 'recommendations?uid=0Ufxf3mvXdR8qfmJGgFImEdCH3f2')).data.map(d => d.uid);
    for (let uid of uids) {
        console.log(uid);
        await axios.post(API_URL + 'story?uid='+uid, {story: {title: Sentencer.make('{{ an_adjective }} {{ noun }}'),
            description: Sentencer.make('My {{ adjective }} destination where I saw so many {{ adjective }} {{ nouns }}!'),
